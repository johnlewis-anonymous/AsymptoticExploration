...
bool PointCloudLocalization::LoadParameters(const ros::NodeHandle& n) {
  ...
  if (!n.getParam("fiducial_calibration/position/x", init_x))
    b_have_fiducial = false;
  if (!n.getParam("fiducial_calibration/position/y", init_y))
    b_have_fiducial = false;
  if (!n.getParam("fiducial_calibration/position/z", init_z))
    b_have_fiducial = false;
  if (!n.getParam("fiducial_calibration/orientation/x", init_qx))
    b_have_fiducial = false;
  if (!n.getParam("fiducial_calibration/orientation/y", init_qy))
    b_have_fiducial = false;
  if (!n.getParam("fiducial_calibration/orientation/z", init_qz))
    b_have_fiducial = false;
  if (!n.getParam("fiducial_calibration/orientation/w", init_qw))
    b_have_fiducial = false;

  if (!n.getParam("localization/registration_method", params_.registration_method))
    return false;
  if (!n.getParam("localization/compute_icp_covariance",
               params_.compute_icp_covariance))
    return false;
  if (!n.getParam("localization/icp_covariance_method",
               params_.icp_covariance_method))
    return false;
  if (!n.getParam("localization/icp_max_covariance", params_.icp_max_covariance))
    return false;
  if (!n.getParam("localization/compute_icp_observability",
               params_.compute_icp_observability))
    return false;
  if (!n.getParam("localization/tf_epsilon", params_.tf_epsilon))
    return false;
  if (!n.getParam("localization/corr_dist", params_.corr_dist))
    return false;
  if (!n.getParam("localization/iterations", params_.iterations))
    return false;
  if (!n.getParam("localization/transform_thresholding", transform_thresholding_))
    return false;
  if (!n.getParam("localization/num_threads", params_.num_threads))
    return false;
  if (!n.getParam("localization/enable_timing_output",
               params_.enable_timing_output))
    return false;
  if (!n.getParam("localization/max_translation", max_translation_))
    return false;
  if (!n.getParam("localization/max_rotation", max_rotation_))
    return false;
  if (!n.getParam("localization/normal_search_radius",
               params_.k_nearest_neighbours_))
    return false;
  if (!n.getParam("b_is_flat_ground_assumption", b_is_flat_ground_assumption_))
    return false;
  if (!n.getParam("localization/recompute_covariance_local_map",
               recompute_covariance_local_map_))
    return false;

  if (!n.getParam("localization/recompute_covariance_scan",
