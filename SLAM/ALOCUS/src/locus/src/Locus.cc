...
bool Locus::LoadParameters(const ros::NodeHandle& n) {
  ...
  if (!n.getParam("b_verbose", b_verbose_))
    return false;
  if (!n.getParam("odom_pub_rate", odom_pub_rate_))
    return false;
  if (!n.getParam("transform_wait_duration", transform_wait_duration_))
    return false;
  if (!n.getParam("b_add_keyframes_enabled", b_add_keyframes_enabled_))
    return false;
  if (!n.getParam("translation_threshold_kf", translation_threshold_kf_))
    return false;
  if (!n.getParam("rotation_threshold_kf", rotation_threshold_kf_))
    return false;
  if (!n.getParam("map_publishment/meters", map_publishment_meters_))
    return false;
  if (!n.getParam("map_publishment/b_publish_map", b_publish_map_))
    return false;
  if (!n.getParam("mapper/num_threads", mapper_threads_))
    return false;
  if (!n.getParam("frame_id/fixed", fixed_frame_id_))
    return false;
  if (!n.getParam("frame_id/base", base_frame_id_))
    return false;
  if (!n.getParam("frame_id/imu", imu_frame_id_))
    return false;
  if (!n.getParam("frame_id/bd_odometry", bd_odom_frame_id_))
    return false;
  if (!n.getParam("frame_conversions/b_convert_imu_to_base_link_frame",
               b_convert_imu_to_base_link_frame_))
    return false;
  if (!n.getParam("queues/lidar_queue_size", lidar_queue_size_))
    return false;
  if (!n.getParam("queues/imu_queue_size", imu_queue_size_))
    return false;
  if (!n.getParam("queues/odom_queue_size", odom_queue_size_))
    return false;
  if (!n.getParam("buffers/imu_buffer_size_limit", imu_buffer_size_limit_))
    return false;
  if (!n.getParam("buffers/odometry_buffer_size_limit",
               odometry_buffer_size_limit_))
    return false;
  if (!n.getParam("data_integration/mode", data_integration_mode_))
    return false;
  if (!n.getParam("b_enable_computation_time_profiling",
               b_enable_computation_time_profiling_))
    return false;
  if (!n.getParam("b_run_with_gt_point_cloud", b_run_with_gt_point_cloud_))
    return false;
  if (!n.getParam("gt_point_cloud_filename", gt_point_cloud_filename_))
    return false;
  if (!n.getParam("publish_diagnostics", publish_diagnostics_))
    return false;
  if (!n.getParam("window_local_mapping_type", window_local_mapping_type_))
    return false;
  if (!n.getParam("b_enable_msw", b_enable_msw_))
    return false;
  if (!n.getParam("box_filter_size", box_filter_size_))
    return false;
  if (!n.getParam("velocity_buffer_size", velocity_buffer_size_))
    return false;
  if (!n.getParam("translation_threshold_msw", translation_threshold_msw_))
    return false;
  if (!n.getParam("rotational_velocity_threshold", rotational_velocity_threshold_))
    return false;
  if (!n.getParam("translational_velocity_threshold",
               translational_velocity_threshold_))
    return false;
  if (!n.getParam("statistics_time_window", statistics_time_window_))
    return false;
  if (!n.getParam("b_integrate_interpolated_odom", b_integrate_interpolated_odom_))
    return false;
  if (!n.getParam("b_pub_odom_on_timer", b_pub_odom_on_timer_))
    return false;
  if (!n.getParam("b_sub_to_lsm", b_sub_to_lsm_))
    return false;
  if (!n.getParam("xy_cross_section_threshold", xy_cross_section_threshold_))
    return false;
  if (!n.getParam("translation_threshold_closed_space_kf",
               translation_threshold_closed_space_kf_))
    return false;
  if (!n.getParam("rotation_threshold_closed_space_kf",
               rotation_threshold_closed_space_kf_))
    return false;
  if (!n.getParam("translation_threshold_open_space_kf",
               translation_threshold_open_space_kf_))
    return false;
  if (!n.getParam("rotation_threshold_open_space_kf",
               rotation_threshold_open_space_kf_))
    return false;
  if (!n.getParam("b_debug_transforms", b_debug_transforms_))
    return false;
  if (!n.getParam("wait_for_odom_transform_timeout",
               wait_for_odom_transform_timeout_))
    return false;

  // Adaptive filtering variable
  if (!n.getParam("b_adaptive_input_voxelization", b_adaptive_input_voxelization_))
    return false;
  if (!n.getParam("points_to_process_in_callback", points_to_process_in_callback_))
    return false;

  // Dynamic Switching
  if (!n.getParam("sensor_health_timeout", sensor_health_timeout_))
    return false;
...
void Locus::PublishOdometry(const geometry_utils::Transform3& odometry,
  ...
  transformStamped.header.frame_id = fixed_frame_id_; // Replace with your parent frame
  transformStamped.child_frame_id = base_frame_id_;   // Replace with your child frame
  ...
  // Set the rotation
  transformStamped.transform.rotation = odometry_msg.pose.pose.orientation;

  // Broadcast the transform
  broadcast_map_to_base.sendTransform(transformStamped);
}
