...
bool PointCloudMerger::LoadParameters(const ros::NodeHandle& n) {
  if (!n.getParam("merging/number_of_velodynes", number_of_velodynes_))
    return false;
  if (!n.getParam("merging/decimate_percentage", decimate_percentage_))
    return false;
  if (!n.getParam("merging/b_use_random_filter", b_use_random_filter_))
    return false;
  if (!n.getParam("merging/b_use_radius_filter", b_use_radius_filter_))
    return false;
  if (!n.getParam("merging/radius", radius_))
    return false;
  if (!n.getParam("merging/radius_knn", radius_knn_))
    return false;
  return true;
}
