...
bool PointCloudIkdTreeMapper::Initialize(const ros::NodeHandle& n)
{
  ...
  ROS_INFO("Registered CAllabacks");
...
bool PointCloudIkdTreeMapper::LoadParameters(const ros::NodeHandle& n)
{
  ...
  if (!n.getParam("frame_id/lidar", lidar_frame_id_))
        return false;
  if (!n.getParam("frame_id/world", world_frame_id_))
        return false;
    
  // Load map parameters.
  if (!n.getParam("map/octree_resolution", octree_resolution_))
    return false;
  
  if (!n.getParam("map/b_publish_only_with_subscribers", b_publish_only_with_subscribers_))
    return false;
...
bool PointCloudIkdTreeMapper::RegisterCallbacks(const ros::NodeHandle& n)
{
  ...
  m_markerFrontierPub = nl.advertise<visualization_msgs::MarkerArray>("frontier_cells_vis_array", 10, false);
