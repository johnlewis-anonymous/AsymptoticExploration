...
bool AstarPlanner::LoadParameters(const ros::NodeHandle& n)
{
  if (!n.getParam("safe_obstacle_distance", safe_obstacle_distance))
        {
            ROS_INFO("Failed to load safe_obstacle_distance");
            return false;
        }
    if (!n.getParam("euclidean_distance_cutoff", euclidean_distance_cutoff))
        {
            ROS_INFO("Failed to load euclidean_distance_cutoff");
            return false;
        }
    if (!n.getParam("submap_distance", submap_distance))
        {
            ROS_INFO("Failed to load submap_distance");
            return false;
        }
    if (!n.getParam("planning_tree_resolution", planning_tree_resolution))
        {
            ROS_INFO("Failed to load planning_tree_resolution");
            return false;
        }
    if (!n.getParam("distance_penalty", distance_penalty))
        {
            ROS_INFO("Failed to load distance_penalty");
            return false;
        }
    if (!n.getParam("greedy_penalty", greedy_penalty))
        {
            ROS_INFO("Failed to load greedy_penalty");
            return false;
        }
    if (!n.getParam("timeout_threshold", timeout_threshold))
        {
            ROS_INFO("Failed to load timeout_threshold");
            return false;
        }
    if (!n.getParam("max_waypoint_distance", max_waypoint_distance))
        {
            ROS_INFO("Failed to load max_waypoint_distance");
            return false;
        }
    if (!n.getParam("min_altitude", min_altitude))
        {
            ROS_INFO("Failed to load min_altitude");
            return false;
        }
    if (!n.getParam("max_altitude", max_altitude))
        {
            ROS_INFO("Failed to load max_altitude");
            return false;
        }
    if (!n.getParam("unknown_is_occupied", unknown_is_occupied))
        {
            ROS_INFO("Failed to load unknown_is_occupied");
            return false;
        }
...
// void AstarPlanner::SetAstarPlannerParameters(double safe_obstacle_distance, double euclidean_distance_cutoff, double submap_distance, double planning_tree_resolution,
//                            double distance_penalty, double greedy_penalty, double timeout_threshold, double max_waypoint_distance, double min_altitude,
//                            double max_altitude, bool unknown_is_occupied) {
